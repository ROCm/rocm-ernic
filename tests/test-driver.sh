#!/bin/bash
#
# test-driver.sh - Clean test of rocm_ernic server and driver
#
...
sudo pkill -9 -f rocm_ernic 2>/dev/null || true
...
# Step 2: Start server
echo "Step 2: Starting rocm_ernic server..."
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
cd "$PROJECT_DIR"
sudo ./build/rocm_ernic --socket /tmp/vfio-user-rocm-ernic.sock --verbose 2>&1 | tee /tmp/vfu_server.log &
SERVER_PID=$!
sleep 3
...
echo "To stop: sudo pkill -9 -f 'qemu-system|rocm_ernic'"
