...
static int setup_context(struct test_context *test_ctx)
{
    ...
    ASSERT(test_ctx->send_buf && test_ctx->recv_buf,
...
static int test_send_recv(struct test_context *test_ctx, size_t size)
{
    ...
    while ((!send_done || !recv_done) && poll_count < 1000) {
        ...
        if (ret > 0) {
            ASSERT(wc.status == IBV_WC_SUCCESS, "Send completion failed: %s",
                   ibv_wc_status_str(wc.status));
            ASSERT(wc.wr_id == 2, "Wrong send WR ID");
            ASSERT(wc.byte_len == size, "Wrong send byte_len: %u != %zu",
                   wc.byte_len, size);
            send_done = 1;
        }
        ...
        if (ret > 0) {
            ASSERT(wc.status == IBV_WC_SUCCESS, "Recv completion failed: %s",
                   ibv_wc_status_str(wc.status));
            ASSERT(wc.wr_id == 1, "Wrong recv WR ID");
            ASSERT(wc.byte_len == size, "Wrong recv byte_len: %u != %zu",
                   wc.byte_len, size);
            recv_done = 1;
        }
    ...
    ASSERT(send_done && recv_done,
           "Timeout waiting for completions (send=%d, recv=%d)", send_done,
           recv_done);
...
{
    if (test_ctx->qp)
        ibv_destroy_qp(test_ctx->qp);
    if (test_ctx->send_mr)
        ibv_dereg_mr(test_ctx->send_mr);
    if (test_ctx->recv_mr)
        ibv_dereg_mr(test_ctx->recv_mr);
    if (test_ctx->send_cq)
        ibv_destroy_cq(test_ctx->send_cq);
    if (test_ctx->recv_cq)
        ibv_destroy_cq(test_ctx->recv_cq);
    if (test_ctx->pd)
        ibv_dealloc_pd(test_ctx->pd);
    if (test_ctx->ctx)
        ibv_close_device(test_ctx->ctx);
    if (test_ctx->dev_list)
        ibv_free_device_list(test_ctx->dev_list);
