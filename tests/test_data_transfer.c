...
static int poll_completions(struct test_context *ctx, int expected_sends, int expected_recvs)
{
    struct ibv_wc wc;
    int total_polled = 0;
    int attempts = 0;
    ...
    if (ctx->completed_sends < expected_sends || 
        ...
        fprintf(stderr, "Timeout: sends=%d/%d, recvs=%d/%d\n",
                ctx->completed_sends, expected_sends,
                ctx->completed_recvs, expected_recvs);
        return -1;
    }
...
static int test_basic_send_recv(struct test_context *ctx)
{
    ...
    /* Note: With loopback backend in "preserve" mode, data isn't actually copied */
    /* The loopback backend only simulates the operation */
    printf("\nNote: Loopback backend simulates data transfer\n");
    printf("      Actual data verification depends on backend pattern configuration\n");
