...
static void activate_device(PVRDMADev *dev)
{
    ...
    if (ret == 0) {
        ...
        rdma_info_report("activate_device: Failed to add default GID (ret=%d)", ret);
    }
}
                           ...
                           unsigned size)
{
    ...
    case PVRDMA_UAR_QP_OFFSET:
        rdma_info_report(">>> UAR: QP doorbell! val=0x%lx, flags: send=%d recv=%d",
                         (unsigned long)val,
                         !!(val & PVRDMA_UAR_QP_SEND),
