...
void *pci_dma_map(PCIDevice *dev, dma_addr_t addr, dma_addr_t *plen, int dir)
{
    ...
    if (ret < 0) {
        rdma_error_report("Failed to convert address %#lx to SGL: %s", addr,
                          strerror(errno));
        free(sg);
        *plen = 0;
        return NULL;
    ...
    if (ret < 0) {
        rdma_error_report("Failed to map SGL for address %#lx: %s", addr,
                          strerror(errno));
        free(sg);
        *plen = 0;
        return NULL;
    ...
    *plen = iov.iov_len; /* Update with actual mapped length */
    ...
    rdma_debug_report("DMA map: guest=%#lx -> host=%p len=%zu", addr, host_addr,
                      (size_t)*plen);
