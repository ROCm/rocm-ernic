 ...
 *
 * Command flow:
 *   1. ionic.ko reads dev_info_regs.signature; must be IONIC_DEV_INFO_SIGNATURE.
 ...
 *   4. ionic.ko reads comp, reads data, proceeds to LIF_IDENTIFY, LIF_INIT, etc.
 ...
 * -------------------------------------------------------------------------
 */
static inline uint16_t le16(uint16_t v) { return htole16(v); }
static inline uint32_t le32(uint32_t v) { return htole32(v); }
static inline uint64_t le64(uint64_t v) { return htole64(v); }
...
static void handle_identify(struct ionic_eth_emu *emu,
                            const uint8_t *cmd, uint8_t *comp, uint8_t *data);
static void handle_lif_identify(struct ionic_eth_emu *emu,
                                const uint8_t *cmd, uint8_t *comp,
                                uint8_t *data);
static void handle_lif_init(struct ionic_eth_emu *emu,
                            const uint8_t *cmd, uint8_t *comp);
static void handle_lif_setattr(struct ionic_eth_emu *emu,
                               const uint8_t *cmd, uint8_t *comp);
static void handle_lif_getattr(struct ionic_eth_emu *emu,
                               const uint8_t *cmd, uint8_t *comp);
static void handle_q_identify(struct ionic_eth_emu *emu,
                              const uint8_t *cmd, uint8_t *comp,
                              uint8_t *data);
static void handle_q_init(struct ionic_eth_emu *emu,
                          const uint8_t *cmd, uint8_t *comp);
static void handle_rdma_cmd(struct ionic_eth_emu *emu,
                            const uint8_t *cmd, uint8_t *comp);
 ...
 * -------------------------------------------------------------------------
 */
ssize_t ionic_eth_emu_bar0_access(struct ionic_eth_emu *emu,
                                  ...
                                  loff_t offset, bool is_write)
{
    ...
    if (offset <= doorbell_off &&
        offset + (loff_t)count > doorbell_off) {
        uint32_t db;
 ...
 * -------------------------------------------------------------------------
 */
ssize_t ionic_eth_emu_bar2_access(struct ionic_eth_emu *emu,
                                  char *buf, size_t count,
                                  loff_t offset, bool is_write)
{
    ...
    if (count == 8) {
        ...
        uint32_t qid     = (uint32_t)(((db >> 24) & 0xffu) |
                                      (((db >> 32) & 0xffffu) << 8));
        ...
        /* AQ doorbell: update producer index so the poll loop knows WQEs are ready.
         * IONIC_RDMA_QTYPE_AQ = 5; each AQ is identified by qid (0-based index). */
...
static void process_devcmd(struct ionic_eth_emu *emu)
{
    ...
    vfu_log(emu->vfu_ctx, LOG_INFO,
            "ionic_eth_emu: devcmd opcode=%u", opcode);
    ...
    default:
        vfu_log(emu->vfu_ctx, LOG_WARNING,
                "ionic_eth_emu: unknown opcode=%u", opcode);
                                ...
                                uint8_t *data)
{
     ...
     * cross-check: words[478] * 4 = 1912 bytes total; 1912 - 392 = 1520 for rdma+pad.
     */
...
#define QTYPE_SZ 12
    ...
    u = le32(IONIC_EMU_AQ_COUNT);  memcpy(aq + 4, &u, 4);
    u = le32(0);                   memcpy(aq + 8, &u, 4);

    /* sq_qtype at rdma+44 */
    uint8_t *sq = rdma + 32 + QTYPE_SZ;
    sq[0] = IONIC_RDMA_QTYPE_SQ;
    u = le32(IONIC_EMU_QP_COUNT);  memcpy(sq + 4, &u, 4);
    u = le32(0);                   memcpy(sq + 8, &u, 4);

    /* rq_qtype at rdma+56 */
    uint8_t *rq = rdma + 32 + 2 * QTYPE_SZ;
    rq[0] = IONIC_RDMA_QTYPE_RQ;
    u = le32(IONIC_EMU_QP_COUNT);  memcpy(rq + 4, &u, 4);
    u = le32(0);                   memcpy(rq + 8, &u, 4);

    /* cq_qtype at rdma+68 */
    uint8_t *cq = rdma + 32 + 3 * QTYPE_SZ;
    cq[0] = IONIC_RDMA_QTYPE_CQ;
    u = le32(IONIC_EMU_CQ_COUNT);  memcpy(cq + 4, &u, 4);
    u = le32(0);                   memcpy(cq + 8, &u, 4);

    /* eq_qtype at rdma+80 */
    uint8_t *eq = rdma + 32 + 4 * QTYPE_SZ;
    eq[0] = IONIC_RDMA_QTYPE_EQ;
    u = le32(IONIC_EMU_EQ_COUNT);  memcpy(eq + 4, &u, 4);
    u = le32(0);                   memcpy(eq + 8, &u, 4);
                            ...
                            const uint8_t *cmd, uint8_t *comp)
{
    ...
    vfu_log(emu->vfu_ctx, LOG_INFO,
            "ionic_eth_emu: LIF_INIT lif_index=%u", lif_index);
}
                               ...
                               const uint8_t *cmd, uint8_t *comp)
{
    ...
    uint8_t attr = cmd[2];
    vfu_log(emu->vfu_ctx, LOG_DEBUG,
            "ionic_eth_emu: LIF_SETATTR attr=%u", attr);
    comp[0] = 0;
}
                               ...
                               const uint8_t *cmd, uint8_t *comp)
{
    ...
    uint8_t attr = cmd[2];
    vfu_log(emu->vfu_ctx, LOG_DEBUG,
            "ionic_eth_emu: LIF_GETATTR attr=%u", attr);
    comp[0] = 0;
}
                              ...
                              uint8_t *data)
{
    ...
    vfu_log(emu->vfu_ctx, LOG_DEBUG,
            "ionic_eth_emu: Q_IDENTIFY qtype=%u", qtype);
                          ...
                          const uint8_t *cmd, uint8_t *comp)
{
    ...
    vfu_log(emu->vfu_ctx, LOG_INFO,
            "ionic_eth_emu: Q_INIT qtype=%u index=%u", qtype, index);

    /* ionic_q_init_comp layout:
     *   [0]   u8   status
     *   [1]   u8   rsvd
     *   [2:3] le16 comp_index   (descriptor ring index for this completion)
     *   [4:7] le32 hw_index     (hardware queue ID; driver stores as q->hw_index
     *                            and uses it for doorbell base address in BAR2)
     *   [8]   u8   hw_type      (hardware queue type for doorbell slot selection)
     *   [9:14]u8   rsvd2[6]
     *   [15]  u8   color
     *
