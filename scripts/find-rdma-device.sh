...
# is rewritten twice: the kernel registers ionic_%d (or rocm_ernic%d on
# the deprecated driver), 60-rdma-persistent-naming.rules rewrites that
# to rocep<bus>s<slot> via rdma_rename, and udev/99-rocm-ernic.rules
# rewrites it again to rocm-rdma-ernic<n>.  Any list of name patterns is
# a snapshot of one moment in that sequence and goes stale silently.
# 0x1022 holds at every step, and is what 99-rocm-ernic.rules keys on.
#
