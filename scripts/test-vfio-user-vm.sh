...
# Configuration
SOCKET_PATH=${SOCKET_PATH:-/tmp/vfio-user-pvrdma-test.sock}
VM_NAME=${VM_NAME:-stebates-test-vm}
VFU_PVRDMA="${PROJECT_ROOT}/build/vfu_pvrdma"
SERVER_LOG="/tmp/vfu-pvrdma-server-$$.log"
...
cleanup() {
    ...
    if [ -n "$SERVER_PID" ] && kill -0 $SERVER_PID 2>/dev/null; then
        echo "Stopping vfu_pvrdma server (PID $SERVER_PID)..."
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  vfu_pvrdma + QEMU VM Test                                   ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""

# Check if vfu_pvrdma is built
if [ ! -x "$VFU_PVRDMA" ]; then
    echo "ERROR: vfu_pvrdma not found or not executable: $VFU_PVRDMA"
...
# Start vfu_pvrdma server
echo ""
echo "=== Starting vfu_pvrdma Server ==="
